...
<head>
<%
	String swfName = (String) request.getParameter("swfName");
...
<script language="javascript">
 ...
 if(!FlashDetect.installed){
    ...
    function goBack(uri){
        clearTimer();
        window.location='<%=ServerUtils.getContextName(request)%>/'+uri;
    }
    function goHome(){
        clearTimer();
...
<body style="margin:0px">
    <object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"
            id="alertrouteMapping" width="101.2%" height="100%">
            <param name="movie" value="swf/admin/<%=ESAPI.encoder().encodeForHTML(swfName)%>.swf<%=ESAPI.encoder().encodeForHTML(swfParams)%>" />
            <param name="quality" value="high" />
            <param name="bgcolor" value="#FFFFFF" />
            <param name="wmode" value="transparent" />
            <param name="allowScriptAccess" value="sameDomain" />
            <embed src="swf/admin/<%=ESAPI.encoder().encodeForHTML(swfName)%>.swf<%=ESAPI.encoder().encodeForHTML(swfParams)%>" quality="high" bgcolor="#FFFFFF"
                width="100%" height="100%" name="alertrouteMapping" align="middle"
                play="true"
                loop="false"
                quality="high"
                wmode="transparent"
