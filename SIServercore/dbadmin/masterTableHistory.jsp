...
<title>View <bean:write property="displayModelName" name="masterTableHistoryForm" /></title>


<script type="text/JavaScript">
    function openPopupToViewRecord(idvalue){
        window.open ("./masterTableRecordHistoryAction.do?selectedId=" +idvalue , "viewTable","menubar=0,resizable=1,scrollbars=1,width=400,height=400");
        return true;
    }
...
function openInfoframe(idvalue, rowObject, clickedRadioId){
    ...
    try{
          ...
          var urlToOpen = "./masterTableRecordHistoryAction.do?selectedId=" +idvalue;
          
          infoFrameWin = new Window({id: "infoFrameWin", className: "alphacube", width:(screen.width)/(3/2), height:(screen.height)/2,top:100,left:100, title:" <bean:write property="displayModelName" name="masterTableHistoryForm" />", closable:true, wiredDrag: true, url: urlToOpen}); 
...
function setSelectedId(value){    
	   document.getElementById("selectedRecordId").value = value;	
}
 ...
 function deleteDS(){
    ...
    function previousPage(){
        ...
        if(parseInt(pageNo)>1){
            document.masterTableHistoryForm.pageNo.value = (parseInt(pageNo)-1);
    ...
    function gotoPage(pageNo){
            document.masterTableHistoryForm.pageNo.value = pageNo;
            document.forms[0].action="./masterTableHistoryAction.do";
            document.forms[0].submit();
    }
    ...
    function submitForm(btn) {
        ...
        if(listSize <= 1){
            ...
            if(btn.id == 'idRestoreUpdate'){ 
	    	     showEmptyDialog('<bean:message key="dbconfig.no.record.history"/>','Alert');
	    	     return false;
            }
        }
        else{
            if(btn.id == 'idDelete' || btn.id == 'idModify' || btn.id=='idRestore' || btn.id == 'idRestoreUpdate'){
...
<script>
           ...
           function operator(display,isString,isNum,isDate,isBln,value) {
              ...
              function addSearchRow() {
                        ...
                        var strHtml = "<select name='"+fieldId+"'  id='"+fieldId+"'  dojoType='dijit.form.FilteringSelect' trim='value' autocomplete='off' class='selectbox'  store='fieldStore' searchAttr='label' onchange='loadOperator(this.id,this.value)'> <option value=''></option> </select>";
                        
                        td1.innerHTML = strHtml;

                        var strHtml1 = "<select name='"+operatorId+"'  id='"+operatorId+"'   trim='value' dojoType='dijit.form.FilteringSelect' autocomplete='off' class='selectbox' store='operatorStore'  searchAttr='label' onchange='displayField(this.id,this.value)'> <option value=''></option> </select>";
                                                
                        td2.innerHTML = strHtml1;

                        var userInput="<div id='searchDataDiv"+count+"' ><input type='text' id='"+searchDataId+"' name='"+searchDataId+"'  dojoType='dijit.form.ValidationTextBox' autocomplete='off' trim='true' ucfirst='true' /></div>";
...
<html:form action="/masterTableHistoryAction" method="POST" >
...
var listSize = '<%= listArray.length %>';
	...
	<input type="hidden" name="selectedRecordId"
        value="<%=ESAPI.encoder().encodeForHTML(request.getParameter("selectedRecordId")!=null? request.getParameter("selectedRecordId"):"")%>" />
	<input type="hidden" name="selectedRecordParentId"
        value="<%=ESAPI.encoder().encodeForHTML(request.getParameter("selectedRecordParentId")!=null? request.getParameter("selectedRecordParentId"):"")%>" />
...
<td>  
 <table cellspacing="0" cellpadding="0">
    ...
    <td class="pageTitle paddingTitle">
         <table>         
          <tr>
              <td height="60px" align="left" valign="top"  class="redtitle"><strong class="pageTitle" style="padding-left:0px"><bean:message key="dbconfig.view"/> <bean:write property="displayModelName" name="masterTableHistoryForm" /><br />
                 <span class="paddingTitleDesc bodytext" style="padding-left:0px"><bean:message key="dbconfig.history.action.description"/> <bean:write property="displayModelName" name="masterTableHistoryForm" /></span></strong>
         ...
         <tr>   
              ...
              <td >&nbsp;</td>      
                           ...
                           <td>
                             <table width="650px">
                              <tr>
                                      ...
                                      <td width="50%" align="right"> <span id="minimize" class="label" <%=(!searchAvailable)?"style='display:\"none\"'":""%> > Minimize <img  onClick="hideSearch()" style="cursor: pointer"  src="<%=ServerUtils.getContextName(request)%>/images/portal/uparrow.gif" width="7" height="11" align="absmiddle" /></span><span id="maximize"  class="label" <%=(searchAvailable)?"style='display:\"none\"'":""%>> Maximize <img style="cursor: pointer" onClick="showSearch()" src="<%=ServerUtils.getContextName(request)%>/images/portal/downarrow.gif" width="7" height="11" align="absmiddle" /></span></td>
                              </tr>
                             </table> 
                           </td>      
                        </tr>
                                                                        
                        <tr id="searchDetail" class="tableBgColor" <%=(!searchAvailable)?"style='display:\"none\"'":""%>>                       
                           <td align="left">
                           ...
                           for (String searchField : searchFields) {
                             ...
                             %>
                                <tr id="searchRow<%=count%>">
                                    ...
                                    <td>
                                      ...
                                      <div id="searchDataDiv<%=count%>" style="display:<%=(dataType.equals("string") || dataType.equals("number"))?"''":"'none'"%>">
                                            <input type="text" id="searchData<%=count%>"
                                            name="searchData<%=count%>" autocomplete="off"
                                            dojoType="dijit.form.ValidationTextBox" required="true"
                                            trim="true" ucfirst="true" />
                                      </div>     
                                        
                                        <div id="searchDataDateDiv<%=count%>" style="display:<%=dataType.equals("date")?"''":"'none'" %>" >
                                          <input id="searchDataDate<%=count%>" type="text" autocomplete="off" style="width:194px;height:1.7em;" dojoType="dijit.form.DateTextBox" datePattern="MM/dd/yyyy" trim="true"  onChange="dojo.byId('calendarDiv<%=count%>').innertHTML=arguments[0];"   invalidMessage="<bean:message key="dbconfig.search.calender.format"  />" />
                                        </div>
                                ...
                                <script>
                                 ...
                                 searchOptions.push(new searchOption('<%=ESAPI.encoder().encodeForHTML(fieldName)%>','<%=ESAPI.encoder().encodeForHTML(dataType)%>','<%=ESAPI.encoder().encodeForHTML(operator)%>','<%=ESAPI.encoder().encodeForHTML(searchData)%>'));
                                    ...
                                    <td>
                                      ...
                                      <div id="searchDataDiv1"  >
                                        ...
                                        <div id="searchDataDateDiv1" style="display:'none'">
                                          <input id="searchDataDate1" type="text" style="width:194px;height:1.7em;"  dojoType="dijit.form.DateTextBox" trim="true" datePattern="MM/dd/yyyy" onChange="dojo.byId('calendarDiv1').innertHTML=arguments[0];" autocomplete="off"  invalidMessage="<bean:message key="dbconfig.search.calender.format"  />" />
                                        </div>
                      ...
                      %>
                        ...
                        <tr id="searchDetailActions" class="tableBgColor" <%=(!searchAvailable)?"style='display:\"none\"'":""%>>
        ...
        <span style="valign=top"><input type="text" autocomplete="off" id="enteredPageNo" size="5" name="enteredPageNo" style="height:30;width:50"
                    ...
                    value="<bean:write name="masterTableHistoryForm" property="pageNo"/> of <bean:write name="masterTableHistoryForm" property="totalNoOfPages"/>"  disabled="true"/></span> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_add_one_on.png" id="next_on" style="display:none;cursor:pointer" onClick="nextPage()"/> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_add_one_off.png" id="next_off" style="display:none;cursor:pointer"/> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_add_all_on.png" id="end_on" style="display:none;cursor:pointer" onClick="gotoPage('<bean:write name="masterTableHistoryForm" property="totalNoOfPages"/>')" /> <img src="<%=ServerUtils.getContextName(request)%>/images/portal/btn_add_all_off.png" id="end_off" style="display:none;cursor:pointer"/></td>
                            ...
                            %>
                                    ...
                                    <html:hidden name="masterTableHistoryForm" property="compositeFiledNames" value="<%=(String)innnerList.get(0)%>"/>
                            <%          
                            ...
                            %>
                              ...
                              <tr  class="bgcolor" onMouseOver="hover(this);" onMouseOut="unHover(this, 1);" onDblClick="openInfoframe(<%=outIndex-1%>, this, '<%=(innnerList.get(0))%>')">
                                    <td class="textnormal12" align="center" ><html:radio disabled="<%=disableAction%>"  property="selectedRecordId" name="masterTableHistoryForm"  
                                    value= "<%=\"\" + (innnerList.get(0))%>" onclick="setSelectedId(this.value)"/></TD>
                            ...
                            <html:hidden name="masterTableHistoryForm" property="parentId"/>
                            <html:hidden name="masterTableHistoryForm" property="totalNoOfPages"/>
                            <html:hidden name="masterTableHistoryForm" property="pageNo"/>
                            <html:hidden name="masterTableHistoryForm" property="screenCategory"/>
...
<tr class="barColor" align="right" >
                <td colspan='<bean:write name="masterTableHistoryForm" property="size"/>' align = "right" width="1000">
                    ...
                    <%} %>
 					<button dojoType="dijit.form.Button"  id="idBack" name="btnBack" type="button" onClick="gotoBack()"> <bean:message key="dbconfig.back" /> </button>
                  <!--  <button dojoType="dijit.form.Button"  id="idCreate" name="btnCreate" type="button" onClick="return submitForm(this)"> <bean:message key="dbconfig.create" /> </button>-->
                    <!--<button dojoType="dijit.form.Button"  id="idModify" name="btnModify" type="button" onClick="return submitForm(this)" > <bean:message key="dbconfig.modify" /></button>-->
                    <button dojoType="dijit.form.Button"  id="idRestore" name="btnRestore" type="button" onClick="return submitForm(this);"> <bean:message key="dbconfig.restore" /> </button>
                    <button dojoType="dijit.form.Button"  id="idRestoreUpdate" name="btnRestoreUpdate" type="button" onClick="return submitForm(this);"> <bean:message key="dbconfig.restoreupdate" /> </button>
                </td>
</tr>
</table>
 <script>
    dojo.addOnLoad(loadFormValues); 
        function loadFormValues(){
                ...
                if(<bean:write name="masterTableHistoryForm" property="totalNoOfPages"/>  <= 1 && <bean:write name="masterTableHistoryForm" property="pageNo"/> <= 1){
                ...
                }else if(<bean:write name="masterTableHistoryForm" property="totalNoOfPages"/>  > 1 && <bean:write name="masterTableHistoryForm" property="pageNo"/> <= 1){
                         document.getElementById('start_on').style.display='none';
                        document.getElementById('start_off').style.display='';
                        document.getElementById('previous_on').style.display='none';
                        document.getElementById('previous_off').style.display='';
                ...
                }else if(<bean:write name="masterTableHistoryForm" property="totalNoOfPages"/>  > 1 && (<bean:write name="masterTableHistoryForm" property="pageNo"/> > 1 && (<bean:write name="masterTableHistoryForm" property="pageNo"/> != <bean:write name="masterTableHistoryForm" property="totalNoOfPages"/>))){
                        ...
                        document.getElementById('end_on').style.display='';
                        document.getElementById('end_off').style.display='none';
                }else if((<bean:write name="masterTableHistoryForm" property="totalNoOfPages"/>  > 1) && (<bean:write name="masterTableHistoryForm" property="pageNo"/> == <bean:write name="masterTableHistoryForm" property="totalNoOfPages"/>)){                     
                        document.getElementById('start_on').style.display='';
                        document.getElementById('start_off').style.display='none';
                        document.getElementById('previous_on').style.display='';
                        document.getElementById('previous_off').style.display='none';
