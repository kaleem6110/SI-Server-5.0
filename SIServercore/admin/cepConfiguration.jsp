...
<script language="javascript">
 ...
 if(!FlashDetect.installed){
    ...
    function goBack(){
        clearTimer();
        window.location='<%=ServerUtils.getContextName(request)%>/dbadmin.do';
    }
    function goHome(){
        clearTimer();
...
<body style="margin:0px">
    <object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"
            id="CorrelationConfiguration" width="101.2%" height="100%">
            <param name="movie" value="swf/admin/CorrelationConfiguration.swf<%=ESAPI.encoder().encodeForHTML(swfParams)%>" />
            <param name="quality" value="high" />
            <param name="bgcolor" value="#FFFFFF" />
            <param name="wmode" value="transparent" />
            <param name="allowScriptAccess" value="sameDomain" />
            <embed src="swf/admin/CorrelationConfiguration.swf<%=ESAPI.encoder().encodeForHTML(swfParams)%>" quality="high" bgcolor="#FFFFFF"
                width="100%" height="100%" name="CorrelationConfiguration" align="middle"
                play="true"
                loop="false"
                wmode="transparent"
                quality="high"
