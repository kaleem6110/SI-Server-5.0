...
GEHelpers.prototype.createPointPlacemark = function(loc, opt_opts) {
  ...
  if (opt_opts.standardIcon && !opt_opts.icon)
    opt_opts.icon = 'http://maps.google.com/mapfiles/' +
  ...
  if (opt_opts.icon) {
    ...
    iconStyle.getIconStyle().setScale(0.4);
    iconStyle.getLabelStyle().setScale(0.5);
 ...
 */
GEHelpers.prototype.clearFeatures = function(placemarks) {
//alert(" geplugin help -clearFeatures :"+placemarks.length);

if(placemarks.length>0){

	for(var i = 0; i < placemarks.length; i++) {
	
	//alert(" object type :"+placemarks[i]);
