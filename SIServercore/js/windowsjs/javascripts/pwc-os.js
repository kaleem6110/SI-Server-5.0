...
Object.extend(Windows, {
  ...
  minimize: function(id, event) {
    ...
    if (win && win.visible) {
	  ...
	  //Start --Aditya,27/01/2009 1:08pm
	   element.title = win.getTitle(); 
	  var windowTitleName = win.getTitle();
	  if(windowTitleName.length > 6){
		  windowTitleName = windowTitleName.substring(0,5)+'...';
	  }else{
		windowTitleName = win.getTitle();
	  }
	  
      Event.observe(element, "mouseup", Windows.restore);
	  if(id == "map3dWin"){
		  windowMinimizeStatus = true;		 
		  Event.observe(element, "mouseup", showControl);
	  }else{
		  windowMinimizeStatus = false;
	  }
      $(element).update(windowTitleName);
      //End --Aditya,27/01/2009 1:08pm
