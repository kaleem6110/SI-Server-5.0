...
define([
	...
	$.widget('spacetimeinsight.siTreemapChartExportAsCSV',$.spacetimeinsight.siViewerExportViewer,{
		options : {
		downloadDisabledSeries : false,
		downloadDisabledColumn : false,		
		},
		pluginName:"siTreemapChartExportAsCSV",
		...
		_initialize : function(){
			...
			var seriesDetails = window.opener.parent.$si.viewer.treemapChartColumnDetails;

				columnDetails = seriesDetails;
				$.each(columnDetails, function(key,value){
					columnItem.push({text:value,id:value, expanded:true});
					});
			...
			if(seriesDetails){
				$.each(seriesDetails, function(key,value){
					seriesItem.push({text:value,expanded:true,id:value});
			});
